...
const getAllCivilizations = async (req, res) => {
  try {
    const allCivilizations = await Civilization.find({})
      ...
      .populate({ path: "unique_tech", select: "name type image -_id" })
      .populate({
        path: "unique_building",
        select: "name description image -_id",
      });
...
const getCivilizationById = async (req, res) => {
  let query = req.query;
  try {
    const civilizationById = await Civilization.findOne(query)
      .select("-_id")
      .populate({ path: "unique_unit", select: "name description image -_id" })
      .populate({ path: "unique_tech", select: "name type image -_id" })
      .populate({
        path: "unique_building",
        select: "name description image -_id",
...
const getCivilizationsByIds = async (req, res) => {
  let query = req.query;
  try {
    const civilizationsByIds = await Civilization.find(query)
      .sort({ number_id: 1 })
      .select("-_id")
      .populate({
        path: "unique_unit",
